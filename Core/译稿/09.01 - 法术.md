...
本章包含一组选例法术，以及给定等级的范式法术或即兴法术能够达成何种效果的准则。对这些准则而言，法术距程为自身，持续为瞬息，目标为个体。这些都是各自类别中最低的项目，因此所给等级就是仪式、范式或即兴法术能够产生该效果的最低等级。

**法术准则：<br>距程：自身；<br>持续：瞬息；<br>目标：个体**
...
**瞬息：** 法术只持续一瞬，然后消散。在许多情况下，法术效果会在法术本身结束后长期存在。例如，一个把石头从山脚移到山顶的掌控法术可以拥有瞬息持续，但石头会留在山顶。类似考虑也适用于大多数湮灭法术；魔法只持续一瞬，但目标保持被摧毁。这不会给准则增加量级。

虽然一瞬是很短的时间，但它可能略长于一个瞬时。对某些法术而言，它可以长达一个战斗轮（六秒），甚至可能稍长一点，尽管这并不常见。一个单一法术效果在短时间内发生，然后法术停止。

拥有瞬息持续的仪式创生法术会创造出像该类事物中其他个体一样持久的东西。魔法在一瞬后消失，因此不能被解除。这同样适用于仪式治疗法术。注意，非自然之物自然情况下根本不会持续任何时间，因此即使用仪式创造，也只会持续到法术结束。（创造这类事物的创生法术需要万变要件。）
...
如同任何赫尔墨斯法术，防护必须穿透魔法抗力才能生效。像“御魔圆环” (Circular Ward Against Demons) 这样的法术在高等级时更难施展，因为法术等级必须等于或超过任何受影响恶魔的威能数值 (Might Score)，而穿透合计也必须超过它。因此，法师很少学习或施展高于约30级的这类法术版本，而更偏好较低等级、能更直接阻碍目标行动的掌控法术。由于大多数情况下赫尔墨斯防护实际上并不以受其影响的生物为目标，多数法师不能用与超自然存在的奥法连接来增强穿透合计，除非该存在在防护施展时位于圆环之内。
...
桌子、长袍和建筑等人造物稍微复杂一些。它们可以用创生魔法凭空创造，也可以用掌控魔法从原材料制成。使用天然原材料的优势在于，瞬息法术会创造出持久物品：改变在一瞬间完成，但原材料不会在法术到期后变回去。用创生制造的物品只在法术持续期间存在，除非该法术是瞬息仪式。
...
范式或仪式法术可以被设计为允许施法者对产品作出某些改变，不过这通常会给法术等级增加量级，以反映复杂性。例如，*秘塔拔地* (Conjuring the Mystic Tower) 允许施法者决定塔内房间布局。简单决定的难度等级为0，较复杂决定有更高难度等级。把神秘塔的每一层分成两半，难度等级为0；创造复杂迷宫则会有高得多的难度等级，可能为12。施法者只能改变法术设计允许改变的要素；例如，施法者不能改变神秘塔的高度或直径。
...
几乎所有准则的基础距程、持续和目标都是最低类别：自身、瞬息、个体。因此，一个实用法术往往会显著高于该效果准则的等级，因为它拥有更高的距程、持续和目标。也有一些例外，主要是防护准则；这些会在准则中明确注明。如果准则没有指定距程、持续或目标，它们就是自身、瞬息和个体。

技法和型式常以每个词的前两个字母缩写；因此 MuTe 法术属于万变 (Muto) 技法与地 (Terram) 型式。等级常加在缩写之后，例如“绯红爆闪” (Flash of the Scarlet Flames) 变为 CrIg 15。

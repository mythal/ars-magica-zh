...
- 如果准则要求仪式，则该法术必须是仪式。
- 如果该法术是一个瞬息创生法术，并创造出持久存在的事物，则它必须是仪式。
...
如果存在要件，会列在每个范式法术中。（例子见第334页法术“喀耳刻魔咒”。）要件会使法术更难施展，因为它们要求施法者拥有更广博的知识。你必须在要件得分与该法术主要技法或型式得分之间取较低者；如果要件是技法，就与主要技法比较；如果要件是型式，就与主要型式比较。例如，如果你的动物为 6、肉身为 13，并施展带有动物要件的万变肉身法术“喀耳刻魔咒”，你就把自己的肉身得分视为只有 6。注意，如果你的要件碰巧更高（例如你的动物为 15），则使用较低的基础型式（此例中为你的肉身 13）。

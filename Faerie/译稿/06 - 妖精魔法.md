...
仪式等级的妖精仪式和赫尔墨斯魔法一样，每个效果量级需要十五分钟，但不需要源质。然而，妖精仪式无法像赫尔墨斯法术那样产生永久效果；瞬息持续的仪式只会直接结束，目标也会立即恢复到自然且未受影响的状态。只有当仪式涉及某种自然（或超自然）过程，例如老化、治疗或扭曲时，才会产生持久影响。例如，某个强迫角色暂时老化的效果，会使该角色承受每年冬季造成的同类身体状况；如果玩家掷骰失败，角色就会遭受自然后果。
...
>
> 施展妖精仪式的角色，除了可以使用与赫尔墨斯魔法（**ArM5**，第111-114页）和梅里尼塔学派妖精魔法（**ArM5**，第92-93页）相关的距程、持续和目标外，还总是可以使用以下任何距程、持续和目标。
>
...
>
> **持留（持续）：** 这个持续与专注同等级。角色可以照常施行仪式，但只要他保持专注，效果就会延迟。当他释放该效果时，将其视为以瞬息持续施展。
>
